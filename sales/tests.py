...
class FruitTest(TestCase):
    ...
    def test_new_submit(self):
        response = self.client.post('/sale/new/', data={
            'fruit_list': 1,
            'number': 1,
            'sold_at': '2018-12-13T12:13'
        }, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'sales/top.html')
    ...
    def test_new_submit_with_no_login(self):
        ...
        response = client.post('/sale/new/', data={
            'fruit_list': 1,
            'number': 1,
            'sold_at': '2018-12-13T12:13'
        }, follow=True)
    ...
    def test_edit_submit(self):
        response = self.client.post('/sale/1/edit/', data={
            'fruit_list': 1,
            'number': 1,
            'sold_at': '2018-12-14T12:14'
        }, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'sales/top.html')
    ...
    def test_edit_submit_with_no_login(self):
        ...
        response = client.post('/sale/1/edit/', data={
            'fruit_list': 1,
            'number': 1,
            'sold_at': '2018-12-14T12:14'
        }, follow=True)
    ...
    def test_upload_submit(self):
        with open('sales/fixtures/upload.csv') as fr:
            response = self.client.post('/sale/upload/', data={
                'file': fr,
            }, follow=True)
